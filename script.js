...
// script.js (add this to the bottom of the DOMContentLoaded listener or the file)

  // ----- Booking Form Confirmation Modal Logic -----
...
const email = document.getElementById('email');
      ...
      if (!checkIn.value) { markError(checkIn, "Please select a check-in date."); valid = false; }
	  if (!checkOut.value) { markError(checkOut, "Please select a check-out date."); valid = false; }
...
const msg = [
  ...
  `Email: ${email.value}`,
  "",
  "Your booking has been saved successfully.A confirmation email will be sent shortly. Enjoy your trip with GoTravel!"
].join("\n");
